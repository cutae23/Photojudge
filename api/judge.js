// /api/judge.js
// Vercel 서버리스 함수 — Gemini API 키는 우선순위에 따라 결정됩니다:
//   1) 사용자가 직접 입력한 개인 키(req.body.userApiKey)가 있으면 그것을 사용
//   2) 없으면 서버 환경 변수(GEMINI_API_KEY)를 사용
// 어느 경우든 키는 이 서버 함수 내에서만 쓰이고 응답에 포함되지 않습니다.
...
export default async function handler(req, res) {
  ...
  const { images, customCriteria, keywords, judgeId, userApiKey } = req.body || {};
  ...
  if (userApiKey && typeof userApiKey === 'string' && userApiKey.trim()) {
    ...
    if (!trimmed.startsWith('AIza') || trimmed.length < 20) {
      return res.status(400).json({ error: '입력한 개인 API 키 형식이 올바르지 않습니다. (AIza로 시작해야 함)' });
    }
    apiKey = trimmed;
  } else {
  ...
  if (!apiKey) {
    return res.status(500).json({
      error: 'Gemini API 키가 없습니다. 화면 상단에 개인 키를 입력하거나, Vercel 프로젝트 설정 > Environment Variables에서 GEMINI_API_KEY를 추가해주세요.'
    });
  }
...
photoIndex는 1부터 시작하는 사진 번호이며, 업로드된 모든 이미지에 대해 순위를 매겨주세요(전체 순위, rank 1이 최고). reason은 반드시 짧게 작성하세요.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) friendly += ' — API 키 권한 문제입니다. (개인 키를 입력했다면 키가 유효한지 확인해주세요)';
